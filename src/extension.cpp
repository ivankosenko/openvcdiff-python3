...
static PyObject* simple_encode(Py_buffer& dictionary, Py_buffer& target)
{
    std::string delta;
    ...
    return PyBytes_FromStringAndSize(delta.data(), delta.size());
}

static PyObject* hashdict_encode(hasheddictionary* hashdict, Py_buffer& target)
{
...
static PyObject* openvcdiff_encode(PyObject* self, PyObject* args, PyObject *kwargs)
{
    Py_buffer target;
    PyObject* dictionary;
    ...
    /* The s* format actually enforces a continuous buffer, which is not mentioned in the Python documentation. */
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "y*O:encode", const_cast<char**>(keywords), &target, &dictionary)) {
        return NULL;
    }
    ...
    if (Py_TYPE(dictionary) != getHashedDictionaryType()) {
        PyBuffer_Release(&target);
        Py_DECREF(dictionary);
        return PyErr_Format(PyExc_TypeError, "encode() argument 2 must be buffer or HashedDictionary");
    }

    return hashdict_encode((hasheddictionary*)dictionary, target);
}

static PyObject* openvcdiff_decode(PyObject* self, PyObject* args, PyObject* kwargs)
{
    ...
    /* The s* format actually enforces a continuous buffer, which is not mentioned in the Python documentation. */
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "y*y*:decode", const_cast<char**>(keywords), &delta, &dictionary)) {
        return NULL;
    }
    ...
    // Too bad, this is one more unneeded copy: The simple interface wants a string object for the input data...
    ...
    return PyBytes_FromStringAndSize(target.data(), target.size());
}
...
static PyMethodDef OpenVcdiffMethods[] = {
    {"encode", (PyCFunction) openvcdiff_encode, METH_VARARGS | METH_KEYWORDS,
     ...
     "Encodes *target* using the data in *dictionary*. Both can be byte strings or memory views. Returns a memory view with the delta."},
    {"decode", (PyCFunction) openvcdiff_decode, METH_VARARGS | METH_KEYWORDS,
     "decode(delta, dictionary)\n"
     "Applies the given *delta* to the *dictionary* data. Again, both can be byte strings or memory views. Returns a memory view with the reconstructed target."},
...
PyObject* hasheddictionary_new(PyTypeObject* subtype, PyObject* args, PyObject* kwds)
{
    ...
    if (!PyArg_ParseTuple(args, "y*:HashedDictionary", &dictionary))
        return NULL;
...
static struct PyModuleDef openvcdiffmodule = {
    PyModuleDef_HEAD_INIT,
    "openvcdiff",  /* name of module */
    "Binding for https://github.com/google/open-vcdiff", /* module documentation, may be NULL */
    -1,       /* size of per-interpreter state of the module, or -1 if the module keeps state in global variables. */
    OpenVcdiffMethods,
};

PyMODINIT_FUNC
PyInit_openvcdiff(void)
{
    if (PyType_Ready(&HashedDictionaryType) < 0)
        return 0;

    PyObject* module = PyModule_Create(&openvcdiffmodule);
    if (module) {
